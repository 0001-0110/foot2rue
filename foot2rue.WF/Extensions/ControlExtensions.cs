...
namespace foot2rue.WF.Extensions
{
	internal static class ControlExtensions
	{
		...
		private static void FindChildrenOfType<T>(this Control control, ref ICollection<T> controls, bool recursive = false) where T : Control
		{
			foreach (Control child in control.Controls)
			{
		...
		{
			ICollection<TControl> controls = new List<TControl>();
